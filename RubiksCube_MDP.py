...
states of the Rubik's Cube puzzle, as well as its operators,
...
def goal_test2(s):
    in_order_sides = ['a', 'b', 'c', 'aprime', 'bprime', 'cprime']

    curr_side_val = 0
    correct_sides = 0

    for side in in_order_sides:
        is_side_correct = True
        side_arr = s.d[side]
        for val in side_arr:
            if not val == curr_side_val:
                is_side_correct = False
                break
        if is_side_correct:
            correct_sides += 1
...
def T(s, a, sp):  # Here is the heart of the MDP: its transition model.
    ...
    if a == "Exit":
        return 0  # Exit action not allowed anywhere else.
...
def initialize_V_from_QL(v):
    ...
    if not ALL_STATES:
        print("In initialize_V_from_VI, ALL_STATES is None.")
    V_from_QL = {}
...
# TODO: Create our own GUI imported as 'Vis'.
#  All is commented out below because there are many instances
#  of Vis being used to setup and run the GUI
#  ----------------------------------------------------------------------
